...
// with modification by Liam Mann
...
(function () {
      ...
      function router () {
        ...
        //CHANGE: Added the simple ability of having ids in uri string
        var wildcard = /[^/]*$/.exec(url)[0];
        var x = parseInt(wildcard);
        if (Math.floor(x) == x){
          var fakeURI = url.slice(0,-wildcard.length)+':id';
        }
